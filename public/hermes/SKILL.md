...
description: >
  ...
  company cortex to consult its knowledge. Triggered by the word "cortex" — "dump your
  session into your cortex", "check your cortex for X", "ask the MOCA cortex about X".
  Built for the Hermes agent (nousresearch.com).
version: 1.0.0
license: MIT
...
> ```
> It installs as `/cortex`. Then set `CORTEX_BASE_URL` + `CORTEX_API_KEY` in `~/.hermes/.env` (see Connect). Once this skill is published to a Hermes skill hub, `hermes skills install` by name will work too.
...
| **Community** | a curated shared body (e.g. Museum of Crypto Art: web3 & cryptoartists) | recall | `cortex_ro_` | **"the MOCA cortex"** |
...
| **"ask the MOCA cortex about X"** / **"check the team cortex for X"** | same verbs, routed to a **named** source |
...
```bash
S="$(find "$HOME/.hermes/skills" -path "*/cortex/scripts/cortex.sh" | head -1)"
# read-only community cortex — empty collection = query the whole instance:
bash "$S" connect moca https://moca.example.com cortex_ro_xxx "" ro "Museum of Crypto Art"
...
2. Save it (prints the `document_id`), and optionally wait until it's searchable:
   ...
   ```bash
   S="$(find "$HOME/.hermes/skills" -path "*/cortex/scripts/cortex.sh" | head -1)"; bash "$S" save ~/.hermes/skills/state/cortex/outbox/session-<date>.md
   ```
...
```bash
bash "$(find "$HOME/.hermes/skills" -path "*/cortex/scripts/cortex.sh" | head -1)" sync
```
...
```bash
bash "$(find "$HOME/.hermes/skills" -path "*/cortex/scripts/cortex.sh" | head -1)" check "what do I know about X?"
```
...
```bash
bash "$(find "$HOME/.hermes/skills" -path "*/cortex/scripts/cortex.sh" | head -1)" search "X"
```
...
- **Name the cortex you used.** With several sources connected, tell them where an answer came from — "per the **MOCA** cortex…" vs "from **your** cortex" vs "the **team** cortex says…". Provenance is trust.
